...
public class UDPTransport extends Thread {
    ...
    public void init() {
        ...
        try {
            ...
            Controls.getLogger().severe(
                    "Cannot bind to " + "socket on port: "
                            + Integer.valueOf(Controls.UDP_PORT).toString()
                            + " (" + se.getMessage() + ")");
        }
        // Start running
        run = true;
                ...
                if (socket.isBound()) {
                    while (true) {
                        DatagramPacket packet = new DatagramPacket(
                                new byte[Controls.UDP_MTU], Controls.UDP_MTU);
                        try {
    ...
    public void close() {
        ...
        try {
            ...
            Controls.getLogger().severe("Shutdown " + "interrupted!");
        }
        // Close socket
        if (socket != null) {
            socket.close();
    ...
    public void send(PacketOut p) {
        try {
            ...
            Controls.getLogger().fine(
                    "Sendlist put " + "timeout! (" + ie.getMessage() + ")");
        }
    }
}
