...
/**
 * The settings that are saved to a backstore.
 * @author Pal Hargitai
 * @author Siu-Hong Li
 */
public class Settings {
	// The preferences
	Preferences prefs;
	// username, incoming and outgoing stats
	...
	public void quit() {
		try {
			...
			Output.err("Error saving settings");
		}
	}
	...
	public String[] getKeys(String loc) {
		try {
			...
			Output.err("Backing store error!");
		}
		return new String[0];
	}
	 ...
	 */
	public String getValue(String loc, String key, String def) {
		Preferences n = prefs.node(loc);
	 ...
	 * @param val The value.
	 */
	public void setValue(String loc, String key, String val) {
		Preferences n = prefs.node(loc);
		n.put(key, val);
		try {
			n.sync();
